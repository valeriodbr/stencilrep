...
import { DEFAULT_STYLE_MODE, ENCAPSULATION, PRERENDER_VNODE_ID } from '../util/constants';
...
export const initHostSnapshot = (domApi: d.DomApi, cmpMeta: d.ComponentMeta, hostElm: d.HostElement, hostSnapshot?: d.HostSnapshot, attribName?: string) => {
  ...
  if (_BUILD_.slotPolyfill) {
    ...
    if (!hostElm['s-cr'] && !domApi.$getAttribute(hostElm, PRERENDER_VNODE_ID) && (!domApi.$supportsShadowDom || cmpMeta.encapsulationMeta !== ENCAPSULATION.ShadowDom)) {
